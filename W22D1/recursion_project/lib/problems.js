...
function fileFinder(directories, targetFile) {
    ...
    if (directories instanceof String) {
    ...
    for (let i = 0; i < keys.length; i++) {
        if (keys[i] === targetFile || fileFinder(directories[keys[i]], targetFile)) {
            return true;
        }
    }
    return false;
...
function pathFinder(directories, targetFile) {
    if (directories === targetFile || Object.keys(directories).includes(targetFile)) {
        return targetFile;
    } else if (directories instanceof String) { 
        return null;
    }
    ...
    for (let i = 0; i < keys.length; i++) {
        let newDirs = directories[keys[i]];
        if (!newDirs) continue;
        
        let nextPath = pathFinder(newDirs, targetFile);
        if (nextPath) {
            if (nextPath[0] === "/") return keys[i] + nextPath;
            return keys[i] + "/" + nextPath;
        }
    }
    return null;
}
